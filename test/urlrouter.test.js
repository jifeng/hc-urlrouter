...
[http, connect].forEach(function (m, index) {
  ...
  describe(moduleName + '.createServer()', function () {
    ...
    describe('get()', function () {
      ...
      it('should return /mwSuccess', function (done) {
        app.request().get('/mwSuccess').end(function (res) {
          res.should.status(200);
          res.body.toString().should.equal('GET /mwSuccess');
          done();
        });
      });
...
var routerWithCustomHandler = urlrouter(function (app) {
  ...
  errorHandler: function (err, req, res) {
    res.statusCode = 200;
    res.end('oh no, error occurred on ' + req.originalUrl);
  }
});
